...
const Dev: React.FC<Props> = ({ showModal, closeModal, collapseSidebar }) => {
  ...
  const handleOpenYesNoModal = () => {
    showModal({
      ...
      data: {
        title: 'Delete File',
        message: 'Are you sure that you want to delete Document 5_exported.raw?',
        onOk: closeModal,
        onCancel: closeModal,
        // iconVariant: 'warning',
      },
  ...
  const handleOpenConfirmModal = () => {
    showModal({
      type: MODAL_TYPES.emailConfirmationModal,
      data: {
        title: 'Delete File',
        message: 'Are you sure that you want to delete Document 5_exported.raw?',
        onOk: closeModal,
  ...
  const renderBody = () => (
    <View className="mt-8" flexGrow={1}>
      ...
      <View className="mt-40" isRow justify="space-around">
        ...
        <Button icon={<ToolTip message={'Test Tool Tip'} />} iconPosition="right" variant="outline">
          Icon Right
        </Button>
      ...
      <View className="mt-40">
        {_.chunk(generateArray(7, '0'), 4).map((smallArray: any[], idx: number) => (
          <div className="columns" key={idx}>
            {smallArray.map((_, i: number) => (
              <div className="column" key={i}>
                <ImageLazy
                  style={{ height: '200px' }}
                  src={`https://picsum.photos/1000/1000?random=${idx * 5 + i}`}
                />
              </div>
            ))}
          </div>
